...
def train_asm_epochs(model, meta_epochs, inner_meta_epochs=1, prefix='ASM'):
    ...
    for idx, epoch in enumerate(range(start_epoch, start_epoch + meta_epochs)):
        ...
        for i, (input, target, img_idxs) in enumerate(unlabel_loader):
            ...
            asm_data = np.append(results['hc']['data'], uc_data, axis=0)
            asm_targets = np.append(results['hc']['targets'], uc_targets, axis=0)
        ...
        epoch_total_uc_idxs[epoch] = total_uc_idxs
        ...
        # Early stop
        # if np.mean(total_hc_ratios) > 0.9:
        #     break

    dump_json(epoch_total_uc_idxs, f'{exp_dir}/epoch_total_uc_idxs.json')
...
if __name__ == '__main__':
    ...
    if args.ckpt_asm is not None:
        ...
        best_model, best_prec1, best_epoch, global_total_uc_idxs, global_total_hc_idxs = \
            train_asm_epochs(model, meta_epochs, inner_meta_epochs=3, prefix='ASM')
        asm_result = {
